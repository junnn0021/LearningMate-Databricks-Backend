...
@app.post("/items")
async def create_item(id: int, first_name: str, last_name: str, email: str):
    connection = get_db_connection()
    cursor = connection.cursor()
    cursor.execute("INSERT INTO new_table (id, first_name, last_name, email) VALUES (%s, %s, %s, %s)", (id, first_name, last_name, email))
    connection.commit()
    connection.close()
